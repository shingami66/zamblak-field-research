...
| **Current implementation boundary** | The first bounded application slice (commit `0807c7b`) now gives the client one retry-stable key per canonical logical payload, requires and validates the caller-provided key in the Server Action and submit parser, and forwards the validated key to the RPC. The database replay path, same-key conflict check, and unique participation index remain unchanged (`20260723140000_forms_collections_rpcs.sql`; `20260723170000_enforce_one_research_form_per_participation.sql`). |
| **Remaining implementation work** | Preserve the approved database semantics and obtain separate runtime/database evidence; no further application key-generation work is required for this decision. |
