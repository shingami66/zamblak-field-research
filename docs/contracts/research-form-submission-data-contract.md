...
- **Current implementation:** The client generates one key per canonical logical payload and retains it across retries; changed participation/date/material notes rotate the key. The Server Action and submit parser require a valid explicit key and forward it to the RPC (`src/components/forms/CreateResearchFormClient.tsx`; `src/app/forms/new/actions.ts`; `src/lib/forms/input.ts`).
...
| Server action | `isValidUuid(participationId)`; `isValidIsoDate(submittedDate)`; notes trimmed to `null` when blank; explicit idempotency key validated and forwarded | `src/app/forms/new/actions.ts:21-54`; `src/components/forms/CreateResearchFormClient.tsx:101-133` | Partial (rollover dates like `2026-02-31` pass `isValidIsoDate` — DEC-FORM-004; DEC-FORM-003/005 remain separate gaps) |
| RPC/input parse | `parseSubmitResearchFormInput` re-validates UUID + ISO date, normalizes notes to `null`, and rejects missing/malformed keys (8–128 chars) | `src/lib/forms/input.ts:28-61`; `src/lib/idempotency/key.ts` | Partial (DEC-FORM-003/004/005 remain separate gaps; DEC-FORM-006 key validation conforms) |
...
| Application key lifecycle | **Conforming application evidence** (DEC-FORM-006): client key is stable for the canonical logical payload, rotates on material change, and malformed explicit keys fail closed before RPC |
...
| 15 | Retry-stable idempotency key (DEC-FORM-006) | Conforming application evidence | Client operation record and focused retry/rotation tests (`CreateResearchFormClient.tsx`; `CreateResearchFormClient.test.tsx`; `actions.test.tsx`; `rpc.test.ts`) |
...
- `src/app/forms/new/actions.ts` — action validation, explicit idempotency-key forwarding, error mapping
