...
| Research Forms `/forms*` | Yes | **No** | Owner-only routes (non-Owner redirects to `/forbidden`). Live submission/detail/review foundations (`submit_research_form`, `getResearchForm`, `review_research_form`, `/forms/[formId]`); UI remains prototype/sessionStorage-backed; full acceptance runtime is not claimed without valid pricing. |
