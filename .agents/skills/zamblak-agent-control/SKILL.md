...
- **Selective Routing:** Route only skills materially relevant to the immediate task surface (e.g. `zamblak-fieldwork-domain-guard` for respondent registry, participation, eligibility, and research-form domain semantics; `zamblak-product-manager` for product ambiguity or unresolved product choice; `zamblak-security-privacy-guard` for auth, privacy, IDOR, PII, and trust-boundary concerns; `zamblak-db-rls-migration-guard` for schema, migrations, RLS, grants, views, DB functions, and triggers; `zamblak-clean-code-guard` for non-trivial changed production code quality; `zamblak-test-guard` for changed or generated test code quality; `zamblak-supabase-data-engineering` for runtime Supabase client, query, and RPC mechanics; `zamblak-nextjs-framework-engineering` for Next.js framework mechanics; `zamblak-nextjs-performance-engineering` for measured performance bottlenecks; `zamblak-postgres-query-index-guidance` for query/index design; `zamblak-ui-rtl-senior-ux-guard` for Arabic-first RTL, layout, responsive, accessibility, and interaction states; `zamblak-review-delegation` for independent review or session recovery; `zamblak-opencodereview` for OCR review packet preparation). Running tests alone must not automatically activate `zamblak-test-guard`. When multiple domains materially overlap, route the smallest necessary combination. No skill may manufacture task authority.
